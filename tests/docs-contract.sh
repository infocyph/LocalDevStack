...
assert_file_contains "$ai" 'code-only structural extract'
assert_file_contains "$ai" 'incrementally'
assert_file_contains "$ai" '--token-budget 3000'
assert_file_contains "$ai" 'LDS_GRAPHIFY_MAX_RETRY_DEPTH=2'
assert_file_contains "$ai" 'There is no LocalDevStack Graphify HTTP proxy and no Python compatibility script.'
assert_file_contains "$ai" 'Graphify owns the OpenAI-compatible request/response contract'
