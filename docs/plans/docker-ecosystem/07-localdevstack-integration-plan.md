...
Hardware and optional direct-port settings are generated by `lds` only for the current Compose invocation:
...
This keeps the repository at one LLM service definition while still avoiding invalid GPU/device declarations on unsupported hosts.
